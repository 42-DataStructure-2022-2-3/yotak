...
void run_simulation() {
	...
	while (1) {
		...
		if (isSimCustomerQueueFull(data.servicelist) == false
			...
			printf("%d번 고객이 은행 창구에서 업무를 기다리고 있습니다. %d\n", nptr->customer.nbr, needtime);
			free(nptr);
		}
			...
			&& isSimCustomerQueueEmpty(data.servicelist) == false)
		{
			...
			data.windowlist->front->customer.endTime = data.servTime + data.windowlist->front->customer.startTime;
			printf("customer endtime: %d, data.servTime: %d\n", data.windowlist->front->customer.endTime, data.servTime);
